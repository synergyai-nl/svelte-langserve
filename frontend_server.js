// LangServe Socket.IO Frontend Server
...
class LangServeClientManager {
	...
	constructor(private config: LangServeConfig) {
...
class LangServeSocketIOServer {
	...
	constructor(
		server: any,
		langserveConfig: LangServeConfig
	) {
	...
	private async processUserMessage(
		...
		this.io.to(conversationId).emit('message_received', userMessage);
		...
		for (const endpoint of conversation.participants.agents) {
			try {
				...
				this.io.to(conversationId).emit('agent_error', {
	...
	private async streamAgentResponse(
		...
		this.io.to(conversation.id).emit('agent_response_start', {
		...
		try {
			await this.langserveManager.streamEndpoint(
				...
				(chunk: string) => {
					...
					this.io.to(conversation.id).emit('message_chunk', {
						message_id: messageId,
			...
			this.io.to(conversation.id).emit('agent_response_complete', finalMessage);

		} catch (error) {
			this.io.to(conversation.id).emit('agent_response_error', {
				message_id: messageId,
				endpoint_id: endpoint.id,
				error: error.message
			});
	...
	private async invokeAgentResponse(
		...
		try {
			...
			this.io.to(conversation.id).emit('message_received', message);
...
const langserveConfig: LangServeConfig = {
	endpoints: [
		{
			id: 'chatbot',
...
const langserveSocketIO = new LangServeSocketIOServer(server, langserveConfig);

// Health check endpoint
app.get('/health', async (req, res) => {
	try {
		const health = await langserveSocketIO.healthCheck();
		res.json(health);
	} catch (error) {
...
server.listen(PORT, () => {
	console.log(`LangServe Socket.IO Frontend running on port ${PORT}`);
	console.log(`Configured with ${langserveConfig.endpoints.length} LangServe endpoints`);
});

export { LangServeSocketIOServer, LangServeClientManager };

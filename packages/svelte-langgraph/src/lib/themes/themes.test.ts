...
import { flowbiteTheme, flowbiteThemeConfig, createFlowbiteTheme } from './flowbite.js';
import { 
  useTheme, 
  createThemeContext, 
  validateTheme, 
...
describe('Theme Utilities', () => {
  ...
  describe('themeClass', () => {
    ...
    it('should return fallback when condition is false', () => {
      ...
      expect(themeClass(mockTheme, 'sendButton', false, 'fallback')).toBe('fallback');
    });

    it('should return fallback when theme property is missing', () => {
      expect(themeClass(mockTheme, 'nonExistentProperty' as keyof ChatTheme)).toBe('');
      expect(themeClass(mockTheme, 'nonExistentProperty' as keyof ChatTheme, true, 'fallback')).toBe('fallback');
    });
  });

  describe('combineClasses', () => {
    it('should combine theme class with additional classes', () => {
      expect(combineClasses('btn-primary', 'ml-2 mt-1')).toBe('btn-primary ml-2 mt-1');
    });
...
describe('Theme Properties Coverage', () => {
  ...
  it('should have non-empty string values for all theme properties', () => {
    ...
    const optionalProperties = ['sendButtonIcon', 'loadingSpinner', 'inputFieldFocus'];

    for (const theme of themes) {
      for (const [key, value] of Object.entries(theme)) {
        expect(typeof value, `Property ${key} should be a string`).toBe('string');

...
const createLangGraphStore = () => {
  ...
  const connect = (serverUrl: string, userId: string, authToken?: string) => {
    ...
    update((state) => {
      ...
      newSocket.on("conversation_joined", (conversation: Conversation) => {
        update((s) => {
          const existingIndex = s.conversations.findIndex(
      ...
      newSocket.on("message_chunk", (chunk: MessageChunk) => {
        ...
        const timeout = setTimeout(() => {
          streamingLogger.warn(
            "Streaming timeout reached, cleaning up message",
            {
              messageId: chunk.message_id.substring(0, 8) + "...",
      ...
      newSocket.on(
        "assistant_schemas",
        (data: { assistant_id: string; schemas: unknown }) => {
          update((s) => {
      ...
      newSocket.on(
        ...
        (data: { assistant_id: string; healthy: boolean; error?: string }) => {
          ...
          if (!data.healthy && data.error) {
            console.warn(
              `Assistant ${data.assistant_id} is unhealthy:`,
              data.error,
            );
          }
        },
      ...
      newSocket.on(
        ...
        (error: {
          ...
          console.error(
            `Assistant error from ${error.assistant_name}:`,
            error.error,
          );
  ...
  const getAssistantSchemas = (assistantId: string) => {
    update((state) => {
      if (state.socket && state.authenticated) {
        state.socket.emit("get_assistant_schemas", {
  ...
  const getConversationHistory = (conversationId: string) => {
    update((state) => {
      if (state.socket && state.authenticated) {
        state.socket.emit("get_conversation_history", {
          conversation_id: conversationId,
        });
      }
      return state;
...
export const availableAssistants = derived(
  langgraphStore,
  ($store) => $store.availableAssistants,
);
...
export const assistantHealth = derived(
  langgraphStore,
  ($store) => $store.assistantHealth,
);

...
<div class="mb-4 rounded-lg border border-gray-200 p-4">
	...
	{#if $availableEndpoints.length === 0}
		<div class="py-2 text-sm text-gray-500">No endpoints available</div>
	{:else}
		<div class="space-y-3">
				...
				<div class="rounded-md border border-gray-200 p-2">
					<label class="flex cursor-pointer items-start">
						...
						<div class="flex-1">
							...
							<div
								class="text-xs {getEndpointStatus(endpoint.id) === 'healthy'
									? 'text-green-600'
									: getEndpointStatus(endpoint.id) === 'unhealthy'
										? 'text-red-600'
										: 'text-gray-500'}"
							>

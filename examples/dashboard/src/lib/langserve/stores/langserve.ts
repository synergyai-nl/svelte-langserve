...
const createLangServeStore = () => {
  ...
  const { subscribe, set, update } = writable<LangServeState>(initialState);
  
  // Map to store streaming message timeouts
  let streamingTimeouts = new Map<string, NodeJS.Timeout>();
...
export const activeConversationId = derived(langserveStore, $store => $store.activeConversationId);
export const streamingMessages = derived(langserveStore, $store => Array.from($store.streamingMessages.entries()));
export const hasStreamingMessages = derived(langserveStore, $store => $store.streamingMessages.size > 0);

// Derived store for the active conversation
export const activeConversation = derived(
  [langserveStore],
  ([$store]) => $store.conversations.find(c => c.id === $store.activeConversationId)
);
